...
IUSE="custom-cflags cups gnome gnome-keyring gtk3 hidpi hotwording kerberos neon pic +proprietary-codecs pulseaudio selinux +system-ffmpeg +tcmalloc widevine"
...
pkg_setup() {
	...
	CHROMIUM_HOME="/usr/$(get_libdir)/inox-browser${CHROMIUM_SUFFIX}"
...
src_install() {
	...
	newexe out/Release/chrome inox || die

	newexe out/Release/chrome_sandbox chrome-sandbox || die
	fperms 4755 "${CHROMIUM_HOME}/chrome-sandbox"

	newexe out/Release/chromedriver inoxdriver || die
	...
	dosym "${CHROMIUM_HOME}/inox-launcher.sh" /usr/bin/inox-browser${CHROMIUM_SUFFIX} || die
	# keep the old symlink around for consistency
	dosym "${CHROMIUM_HOME}/inox-launcher.sh" /usr/bin/inox${CHROMIUM_SUFFIX} || die

	dosym "${CHROMIUM_HOME}/inoxdriver" /usr/bin/inoxdriver${CHROMIUM_SUFFIX} || die
	...
	for size in 16 22 24 32 48 64 128 256 ; do
		...
		newicon -s ${size} "${branding}/product_logo_${size}.png" \
			inox-browser${CHROMIUM_SUFFIX}.png
	done
	...
	make_desktop_entry \
		inox-browser${CHROMIUM_SUFFIX} \
		"Inox${CHROMIUM_SUFFIX}" \
		inox-browser${CHROMIUM_SUFFIX} \
		"Network;WebBrowser" \
	...
	if use gnome; then
		...
		newins "${FILESDIR}"/inox-browser.xml inox-browser${CHROMIUM_SUFFIX}.xml || die
		if [[ "${CHROMIUM_SUFFIX}" != "" ]]; then
			sed "s:inox-browser:inox-browser${CHROMIUM_SUFFIX}:g" -i \
				"${ED}"/usr/share/gnome-control-center/default-apps/inox-browser${CHROMIUM_SUFFIX}.xml
		fi
	fi
